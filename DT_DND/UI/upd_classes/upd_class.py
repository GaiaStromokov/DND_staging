...
tag = Tag()
def ngen(name): return name.replace(" ", "_")
def agen(name): return name.replace("_", " ")
class upd_class:
    def __init__(self):
    ...
    def Fighter(self):
        ...
        ca = "Second_Wind"
        if ca in data:
            descriptions = [f"(bonus) regain 1d10+{q.db.Core.L} HP"]
    ...
    def Fighter_Samurai(self):
        ...
        if ca in data:
            a = agen(ca)
            cdata = data[a]
            t_header = tag.cfeature.header(ca)
            t_popup = tag.cfeature.popup(ca)
            t_select = tag.cfeature.select(ca, 0)
            with group(parent=self.parent):
                add_text(a, color=c_h1, tag=t_header)
                item_delete(t_popup)
                with popup(t_header, mousebutton=mvMouseButton_Left, tag=t_popup):
                    add_combo(items=get.list_Fighter_Samuri_Bonus_Proficiency, default_value=cdata["Select"][0], width=80, no_arrow_button=True, callback=q.cbh, user_data=["Class Select", ca, 0], tag=t_select)
    ...
    def Wizard(self):
        ...
        if ca in data:
            a = agen(ca)
            t_header = tag.cfeature.header(ca)
            t_tooltip = tag.cfeature.tooltip(ca)
        ...
        if ca in data:
            ...
            with group(parent=self.parent):
                ...
                item_delete(t_popup)
                with popup(t_header, mousebutton=mvMouseButton_Left, tag=t_popup):
                    add_combo(items=q.db.Spell["Book"][1], default_value=cdata["Select"][0], width=80, no_arrow_button=True, callback=q.cbh, user_data=["Class Select", ca, 0], tag=tag.cfeature.select(ca,0))
                    add_combo(items=q.db.Spell["Book"][2], default_value=cdata["Select"][1], width=80, no_arrow_button=True, callback=q.cbh, user_data=["Class Select", ca, 1], tag=tag.cfeature.select(ca,1))
                for idx, spell in enumerate(cdata["Select"]):
                    if spell != "":
                        ...
                        with group(horizontal=True):
                            ...
                            add_checkbox(default_value=cdata["Use"][idx], enabled=True, callback=q.cbh, user_data=["Class Use", ca, idx], tag=tag.cfeature.toggle(ca,idx))
                        item_delete(t_tooltip)
        ...
        if ca in data:
            ...
            with group(parent=self.parent):
                ...
                item_delete(t_popup)
                with popup(t_header, mousebutton=mvMouseButton_Left, tag=t_popup):
                    add_combo(items=q.db.Spell["Book"][3], default_value=cdata["Select"][0], width=80, no_arrow_button=True, callback=q.cbh, user_data=["Class Select", ca, 0], tag=tag.cfeature.select(ca,0))
                    add_combo(items=q.db.Spell["Book"][3], default_value=cdata["Select"][1], width=80, no_arrow_button=True, callback=q.cbh, user_data=["Class Select", ca, 1], tag=tag.cfeature.select(ca,1))
                for idx, spell in enumerate(cdata["Select"]):
                    if spell != "":
                        spell_tag = spell.replace(" ", "_")
                        t_spell = tag.cfeature.text(ca, spell_tag)
    ...
    def Wizard_Abjuration(self):
        ...
        if ca in data:
            ...
            t_header = tag.cfeature.header(ca)
            t_hp = tag.cfeature.button(ca, "HP")
            t_desc = tag.cfeature.text(ca, "Desc")
            with group(parent=self.parent):
                with group(horizontal=True):
                    add_text(a, color=c_h1, tag=t_header)
                    add_checkbox(default_value=cdata["Use"][0], enabled=True, callback=q.cbh, user_data=["Class Use", a, 0], tag=tag.cfeature.toggle(ca, 0))
